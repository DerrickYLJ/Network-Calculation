...
# case 2 : VL2
# VL7 -> VL2 -> VL3 -> VL9 -> switch B (VL7)to A -> VL6 -> VL13 -> VL4 -> VL8 -> VL11 -> switch A (VL7) to ES1

# generate an new system
s = model.System('compare')

# set up all the resouces, or vertexes
VL7 = s.bind_resource(model.Resource("FromSource", schedulers.SPNPScheduler()))
...
t1 = VL7.bind_task(model.Task("T1", wcet=14, bcet=1, scheduling_parameter=2))
...
t4 = VL9.bind_task(model.Task("T4", wcet=121, bcet=1, scheduling_parameter=2))
t5 = switchB_to_A.bind_task(model.Task("T3", wcet=14, bcet=1, scheduling_parameter=2))
t6 = VL4.bind_task(model.Task("T3", wcet=30, bcet=1, scheduling_parameter=2))
...
t9 = VL8.bind_task(model.Task("T3", wcet=14, bcet=1, scheduling_parameter=2))
t10 = VL11.bind_task(model.Task("T3", wcet=121, bcet=1, scheduling_parameter=2))
t11 = switchA_to_ES1.bind_task(model.Task("T4", wcet=14, bcet=1, scheduling_parameter=2))


# specify precedence constraints and link all the tasks altogether:
t1.link_dependent_task(t2).link_dependent_task(t3).link_dependent_task(t4).link_dependent_task(t5).link_dependent_task(t6).link_dependent_task(t7).link_dependent_task(t8).link_dependent_task(t9).link_dependent_task(t10).link_dependent_task(t11)
...
# specify paths
p1 = s.bind_path(model.Path("P1", [t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11]))
