...
# case 2 : VL2
# VL3 -> VL2  -> switch B (VL3)to switch A -> VL6 -> VL5 -> switch A (VL2)to ES1

# generate an new system
s = model.System('compare')

# set up all the resouces, or vertexes
VL3 = s.bind_resource(model.Resource("FromSource", schedulers.SPNPScheduler()))
VL2 = s.bind_resource(model.Resource("VL3", schedulers.SPNPScheduler()))
...
t1 = VL3.bind_task(model.Task("T1", wcet=30, bcet=1, scheduling_parameter=2))
...
t5 = VL4.bind_task(model.Task("T5", wcet=30, bcet=1, scheduling_parameter=2))
