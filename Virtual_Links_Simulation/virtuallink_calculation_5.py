...
# case 2 : VL2
# VL5 -> VL10  -> VL2 -> switch A (VL5)to ES1

# generate an new system
s = model.System('compare')

# set up all the resouces, or vertexes
VL5 = s.bind_resource(model.Resource("FromSource", schedulers.SPNPScheduler()))
VL10 = s.bind_resource(model.Resource("VL10", schedulers.SPNPScheduler()))
...
t1 = VL5.bind_task(model.Task("T1", wcet=8, bcet=1, scheduling_parameter=2))
t2 = VL10.bind_task(model.Task("T2", wcet=30, bcet=1, scheduling_parameter=2))
t3 = VL2.bind_task(model.Task("T3", wcet=2, bcet=1, scheduling_parameter=2))
t4 = switchA_to_ES3.bind_task(model.Task("T4", wcet=8, bcet=1, scheduling_parameter=2))
