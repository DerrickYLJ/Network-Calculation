...
def py_cpa_example():
    ...
    s = model.System('PyCPA_Exp1')

    # add three resources (2 CPUs, 1 Bus) to the system
    # and register the SPP scheduler (and SPNP for the bus)
    CPU1 = s.bind_resource(model.Resource("CPU1", schedulers.SPPScheduler()))
    BUS = s.bind_resource(model.Resource("BUS",  schedulers.SPNPScheduler()))
    CPU2 = s.bind_resource(model.Resource("CPU2", schedulers.SPPScheduler()))

    # create and bind tasks to r1
    t11 = CPU1.bind_task(model.Task("T11", wcet=10, bcet=5, scheduling_parameter=1))
    t12 = CPU1.bind_task(model.Task("T12", wcet=3, bcet=1, scheduling_parameter=2))
    t13 = CPU1.bind_task(model.Task("T13", wcet=5, bcet=2, scheduling_parameter=3))

    # create and bind tasks to r2
    t21 = BUS.bind_task(model.Task("T21", wcet=2, bcet=2, scheduling_parameter=3))
    t22 = BUS.bind_task(model.Task("T22", wcet=9, bcet=5, scheduling_parameter=2))
    t23 = BUS.bind_task(model.Task("T23", wcet=10, bcet=3, scheduling_parameter=1))

    # create and bind tasks to r3
    t31 = CPU2.bind_task(model.Task("T31", wcet=5, bcet=3, scheduling_parameter=2))
    t32 = CPU2.bind_task(model.Task("T32", wcet=3, bcet=2, scheduling_parameter=3))
    t33 = CPU2.bind_task(model.Task("T33", wcet=7, bcet=3, scheduling_parameter=1))
    ...
    t13.link_dependent_task(t23).link_dependent_task(t33)

    # register a periodic with jitter event model for T11 and T12
    t13.in_event_model = model.PJdEventModel(P=30, J=0)
    t12.in_event_model = model.PJdEventModel(P=20, J=0)
    t11.in_event_model = model.PJdEventModel(P=30, J=0)

    # graph the system to visualize the architecture
    g = graph.graph_system(s, filename='%s.pdf' % s.name, dotout='%s.dot' % s.name, show=True)
    ...
    print("1st Result for each task:")
    ...
    p3 = s.bind_path(model.Path("P3", [t13, t23, t33]))

    paths = [p1, p2, p3]
    # perform path analysis of selected 
    print("2nd Path Analysis:")
    for p in paths:
